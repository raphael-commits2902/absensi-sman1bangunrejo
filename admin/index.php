...
$total_siswa = $pdo->query("SELECT COUNT(*) FROM students")->fetchColumn();
$hari_ini = date('Y-m-d');
$hadir_hari_ini = $pdo->query("SELECT COUNT(*) FROM attendance WHERE date = '$hari_ini' AND status = 'Hadir'")->fetchColumn();
$sakit_hari_ini = $pdo->query("SELECT COUNT(*) FROM attendance WHERE date = '$hari_ini' AND status = 'Sakit'")->fetchColumn();
$izin_hari_ini = $pdo->query("SELECT COUNT(*) FROM attendance WHERE date = '$hari_ini' AND status = 'Izin'")->fetchColumn();

// Ambil 5 Absensi Terbaru hari ini
$stmt_recent = $pdo->query("SELECT a.*, s.name, s.class, s.nisn FROM attendance a JOIN students s ON a.student_id = s.id ORDER BY a.date DESC, a.time DESC LIMIT 5");
$recent_attendance = $stmt_recent->fetchAll();
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Dashboard Admin - Absensi SMAN 1 Bangunrejo</title>
    <!-- Google Fonts: Inter -->
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', sans-serif; }
        ...
        .grid-stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 24px; margin-bottom: 35px; }
        ...
        .btn-view-all { font-size: 14px; color: var(--primary); text-decoration: none; font-weight: 600; }
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="sidebar-menu">
            <li><a href="index" class="active"><i class="fa-solid fa-chart-pie"></i> <span>Dashboard</span></a></li>
    ...
    <div class="main-content">
        ...
        <div class="grid-stats">
            ...
            <div class="card-stat stat-yellow">
                <div class="card-info">
                    <p>Siswa Sakit (Hari Ini)</p>
                    <h3><?= $sakit_hari_ini ?></h3>
                </div>
        ...
        <div class="data-panel">
            <div class="panel-header">
                <h3><i class="fa-solid fa-clock-rotate-left" style="color: var(--primary); margin-right: 8px;"></i> Aktivitas Absensi Terbaru</h3>
                <a href="laporan" class="btn-view-all">Lihat Semua Laporan &rarr;</a>
            </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge <?= strtolower($row['status']) ?>">
                                            <?php if($row['status'] == 'Hadir') echo '<i class="fa-solid fa-check"></i>'; ?>
                                            <?php if($row['status'] == 'Sakit') echo '<i class="fa-solid fa-pills"></i>'; ?>
                                            <?php if($row['status'] == 'Izin') echo '<i class="fa-solid fa-envelope"></i>'; ?>
                                            <?= $row['status'] ?>
                                        </span>
                                    </td>
                                </tr>
