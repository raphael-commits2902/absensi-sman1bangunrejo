...
// Ambil nama siswa untuk ditampilkan di Navbar
$user_id = $_SESSION['user_id'];
$stmt_student = $pdo->prepare("SELECT name FROM students WHERE user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(!empty($old_pass) && !empty($new_pass)) {
        try {
            // Ambil data password user saat ini
            $stmt = $pdo->prepare("SELECT password FROM users WHERE id = ?");
            $stmt->execute([$_SESSION['user_id']]);
            $current_hashed_pass = $stmt->fetchColumn();
            ...
            if(password_verify($old_pass, $current_hashed_pass)) {
                ...
                $stmt_update->execute([$new_hashed_pass, $_SESSION['user_id']]);
                
                $success = "Password akun Anda berhasil diperbarui dengan aman!";
            } else {
                $error = "Gagal: Password lama yang Anda masukkan tidak cocok!";
            }
        } catch (PDOException $e) {
             $error = "Terjadi kesalahan pada database: " . $e->getMessage();
        }
    } else {
        $error = "Peringatan: Semua kolom password wajib diisi!";
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Pengaturan Akun - Portal Siswa</title>
    <!-- Google Fonts: Inter -->
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', sans-serif; }
        ...
        /* Form Panel */
        .form-panel { background: white; padding: 35px; border-radius: 20px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); max-width: 500px; }
        ...
        .input-group i { position: absolute; left: 16px; color: var(--gray); font-size: 16px; }
        .input-group input { width: 100%; padding: 12px 16px 12px 45px; border: 2px solid var(--border); border-radius: 10px; font-size: 15px; color: var(--dark-secondary); transition: all 0.3s ease; background: var(--bg-light); outline: none; }
        .input-group input:focus { border-color: var(--primary); background: white; box-shadow: 0 0 0 4px rgba(59, 130, 246, 0.1); }
        .input-group input:focus + i, .input-group input:not(:placeholder-shown) + i { color: var(--primary); }
        ...
        /* Info Box */
        .info-box { background: var(--primary-light); padding: 15px; border-radius: 10px; margin-bottom: 25px; font-size: 13px; color: var(--primary-hover); display: flex; gap: 10px; align-items: flex-start; line-height: 1.5; }
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="pengaturan" class="active"><i class="fa-solid fa-gear"></i> <span>Pengaturan</span></a></li>
        </ul>
    </div>
    ...
    <div class="main-content">
        ...
        <div class="form-panel">
            <h3><i class="fa-solid fa-shield-halved"></i> Keamanan & Password</h3>
            
            <div class="info-box">
                <i class="fa-solid fa-circle-info"></i>
                <p>Ganti password default Anda (NISN) dengan password baru yang lebih kuat. Jangan berikan password Anda kepada siapa pun untuk menghindari penyalahgunaan absen.</p>
            </div>
